...
void firstPass(const char *filename)
{
    ...
    while (fgets(buffer, sizeof(buffer), f))
    {
        ...
        if (buffer[0] == ':')
        {
            ...
            // consume valid identifier characters (no spaces allowed in label name)
            int start = i;
            ...
            // extract just the label name (without the : prefix)
            int labelLen = i - start;
            char labelName[256];
            strncpy(labelName, buffer + start, labelLen);
            ...
            // safe to add label (without the : prefix)
...
int parseCodeLine(char *line, int address)
{
    ...
    while (token && numArgs < 4)
    {
        ...
        if (token[0] == ':')
        {
            // Look up label without the : prefix
            int labelAddress = findLabel(token + 1);
            if (labelAddress != -1)
            {
            ...
            {
                // Label not found - still store it for error checking later
