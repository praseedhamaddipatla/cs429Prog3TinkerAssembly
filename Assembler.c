...
int getRegisterNumber(const char *tok)
{
    ...
    // Check for leading zeros (r00, r01, etc. are invalid)
    if (tok[1] == '0' && tok[2] != '\0')
    {
...
uint64_t convertToNumber(const char *lit)
{
    ...
    // Handle leading zeros by skipping them (treat as decimal)
    const char *parse_ptr = lit;
    if (lit[0] == '0' && lit[1] != 'x' && lit[1] != 'X' && lit[1] != '\0')
    {
        // Skip leading zeros but keep at least one digit
        while (*parse_ptr == '0' && *(parse_ptr + 1) != '\0')
        {
            parse_ptr++;
        }
    }
...
void validateAllInstructions(FILE *in)
{
    ...
    while (fgets(line, sizeof(line), in))
    {
        ...
        if (line[0] == '\t')
        {
            ...
            // Save original for mov validation
            char orig_line[MAX_LINE];
            strcpy(orig_line, buf);

            // Check for balanced parentheses before tokenizing
            int parenCount = 0;
            ...
            if (sec == CODE)
            {
                ...
                else if (strcmp(toks[0], "ld") == 0)
                {
                    ...
                    // Check for overflow BEFORE calling convertToNumber
                    ...
                    {
                        // It's a label, just validate it exists
                ...
                else
                {
                    ...
                    else if (type == PRIV)
                    {
                        ...
                        // priv immediate cannot be a label
                        if (toks[4][0] == ':')
                        {
                            fprintf(stderr, "Error: label not allowed as immediate\n");
                            hadError = 1;
                        ...
                        // Validate priv code is in valid set (0, 3, 4 only per spec)
                    ...
                    else if (type == MOV)
                    {
                        ...
                        // Check if this is a 3-operand mov WITHOUT parentheses (invalid)
                        // mov r0,r0,405 tokenizes to 4 items but has no parens
                        if (n == 4 && strchr(orig_line, '(') == NULL)
                        {
                        ...
                        // mov can have 2 or 3 operands, but NOT in the form "rd, rs, imm"
                        // Valid forms after tokenizing:
                        // n=3: mov rd, rs  OR  mov rd, imm
                        // n=4: mov rd, (rs)(imm)  OR  mov (rd)(imm), rs
                        // n=5: mov (rd)(imm1), (rs)(imm2)

                        if (n == 5)
                        {
                            // mov (rd)(imm1), (rs)(imm2) - tokenized as: mov, rd, imm1, rs, imm2
                            getRegisterNumber(toks[1]);
                            if (hadError)
                                return;

                            uint64_t val1 = convertToNumber(toks[2]);
                            if (hadError)
                                return;
                            ...
                            uint64_t val2 = convertToNumber(toks[4]);
                            if (hadError)
                                return;

                            if (toks[2][0] != ':')
                            {
                                int64_t sval = (int64_t)val1;
                                if (sval < -2048 || sval > 2047)
                                {
                        ...
                        else if (n == 4 && toks[1][0] == 'r' && toks[2][0] == 'r' && toks[3][0] != 'r')
                        {
                            // mov rd, (rs)(imm) - tokenized as: mov, rd, rs, imm
                            getRegisterNumber(toks[1]);
                            if (hadError)
                                return;
                            getRegisterNumber(toks[2]);
                        ...
                        {
                            // mov (rd)(imm), rs - tokenized as: mov, rd, imm, rs
                            getRegisterNumber(toks[1]);
                            if (hadError)
                                return;
...
void firstPass(FILE *in, FILE *mid)
{
    ...
    while (fgets(line, sizeof(line), in))
    {
        ...
        if (line[0] == '.')
        {
            ...
            {
                sec = CODE;
                // DON'T write the directive here
                continue;
            }
            else if (strcmp(line, ".data") == 0)
            {
        ...
        if (line[0] == '\t')
        {
            // Write section directive ONLY when we're about to write content
            if (sec != lastWritten)
            {
                if (sec == CODE)
                {
...
{
    // Reset error flag and label count at start
    hadError = 0;
    numLbls = 0;
